...
class FavoritProvider with ChangeNotifier {
  ...
  Future<void> optimisticAddToCart(String itemId, int quantity, String outletId,
    ...
    if (existingIndex != -1) {
      ...
      _cartItems[existingIndex]['subtotal'] =
          (double.parse(itemData['harga'].toString()) * quantity).toString();
    } else {
      _cartItems.add({
  ...
  Future<bool> addToCart(String itemId, int quantity, String outletId) async {
    ...
    try {
      ...
      // Cetak body sebelum request dikirim
      print('Body: {');
      print('  id_item_menu: $itemId,');
      print('  qty: ${quantity.toString()},');
      print('  id_outlet: $outletId');
      print('}');
