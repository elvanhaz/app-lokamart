...
class DetailLokasiPage extends StatefulWidget {
  const DetailLokasiPage({super.key});

  @override
  State<DetailLokasiPage> createState() => DetailLokasiState();
}

class DetailLokasiState extends State<DetailLokasiPage> {
  // Update lokasi di `LocationProvider` saat lokasi diubah
  void _updateAddress() {
    final locationProvider =
        Provider.of<LocationProvider>(context, listen: false);
    locationProvider.updateLocation(
      _address, // alamat yang sudah diambil dari geocoding
      _getStreetName(_address), // nama tempat
    );
  }

  GoogleMapController? mapController;
  final bool _isFavorite = false;
  final String _addressType = 'Rumah';

  LatLng _currentPosition = const LatLng(0, 0); // Initialize with (0, 0)
  String _address = 'Loading address...';
  final String _detailLokasi = '';
  final String _patokan = '';
  Marker? _marker;
  ...
  void _updatePosition(CameraPosition position) {
    ...
    _debounce = Timer(const Duration(seconds: 2), () {
      if (mounted) _getAddressFromLatLng(_currentPosition);
    });
  }
                      ...
                      SizedBox(
                        ...
                        child: ElevatedButton(
                          onPressed: () {
                            _updateAddress();
                            Navigator.pop(
                                context); // kembali ke halaman checkout
                          },
                          style: ElevatedButton.styleFrom(
                            backgroundColor: Colors.green,
                            shape: RoundedRectangleBorder(
